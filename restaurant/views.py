...
class MenuViewSet(viewsets.ModelViewSet):
    votes_prefetch = Prefetch(
        "votes", queryset=Vote.objects.select_related("user")
    )
    queryset = (
    ...
    def current_day_menus(self, request):
        ...
        menus = Menu.objects.select_related("restaurant").filter(
            post_date=today
        )
...
class VoteViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if Vote.objects.filter(
            user=user, vote_date=timezone.localdate()
        ).exists():
            raise serializers.ValidationError(
                "You have already voted for the menu today!"
            )
